...
//funcion que muestra un enlace al carrito del usuario
function userPedido(){
    $("#user-Pedido").append("<a class='btn-link' role='button' onclick='verCarrito();'>Carrito</a>")
}

//funcion para seguir comprando
function comprarMas()
{
    $.post(base_url + 'controlador/bazar', {}, function(pagina,datos){
...
function verCarrito(){
    $.post(base_url + 'controlador/verCarrito', {}, function(pagina,datos){
       $("#info-center").html(pagina,datos);
    });
}
...
function addProductCar(id) {
    $.post(base_url + 'controlador/addProductCart', {id: id, qty: $("#cantidad" + id).val()},
